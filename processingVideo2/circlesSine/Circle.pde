class Circle {
  ...
  void update() {
    //Sine wave effect
    myX  = (int)map(sin(angle),-1,1,(width/2)-50,(width/2)+50); //Affects X axis, use -angle to invert movement direction
    //myY  = (int)map(sin(angle),-1,1,(width/2)-50,(width/2)+50); //Affects Y axis, use -angle to invert movement direction
    //Change speed of sine wave movement
    angle += 0.01;
